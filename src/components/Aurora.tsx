...
export default function Aurora() {
  ...
  return (
    <div aria-hidden data-aurora className="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
      ...
      {/* Drifting blobs */}
      <motion.div
        className="absolute -left-40 -top-40 h-[42rem] w-[42rem] rounded-full blur-[130px]"
        style={{
          background: isLight
            ? "radial-gradient(circle, rgba(14,165,233,0.35), transparent 65%)"
            : "radial-gradient(circle, rgba(43,184,240,0.20), transparent 65%)",
        }}
        animate={{ x: [0, 80, -30, 0], y: [0, 40, 90, 0] }}
        transition={{ duration: 26, repeat: Infinity, ease: "easeInOut" }}
      />
      <motion.div
        className="absolute right-[-14rem] top-[16rem] h-[38rem] w-[38rem] rounded-full blur-[140px]"
        style={{
          background: isLight
      ...
      />
      <motion.div
        className="absolute bottom-[-16rem] left-1/3 h-[40rem] w-[40rem] rounded-full blur-[150px]"
        style={{
          background: isLight
